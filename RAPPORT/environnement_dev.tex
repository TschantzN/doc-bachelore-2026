...
Le développement du firmware pour le microcontrôleur gérant la transmission (STM32) repose sur 
...
l'émetteur (\texttt{mm-mm8108-ekh05}).
...
# Terminal 1 : Lancement du serveur OpenOCD pour la cible EKH05
openocd -f src/platforms/mm-mm8108-ekh05/openocd.cfg
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
# Terminal 2 : Injection de la configuration dans le Config Store
python ./tools/platform/program-configstore.py -H localhost write-json ..\examples\spi_interface\config.hjson
\end{lstlisting}
\end{enumerate}

\subsubsection{Résolution du conflit de version matérielle et calibration radio}
Lors des premiers déploiements, une erreur bloquait l'initialisation de la communication entre le microcontrôleur 
et la puce Wi-Fi (\texttt{Transport init failed - Chip ID 0x0000}), provoquant un plantage dès la phase d'initialisation 
de la couche d'abstraction matérielle (HAL).

L'analyse de ce blocage a révélé un désaccord dans le ciblage de la carte d'évaluation. La prise en charge officielle de 
notre cible matérielle (\texttt{mm-mm8108-ekh05}) a été finalisée lors d'une mise à jour du SDK Morse Micro. 

Cette mise à jour a permis d'associer le binaire de calibration radio (BCF) adapté à la puce MM8108. Ce fichier 
binaire est directement référencé dans la section \texttt{"files"} du fichier de configuration utilisateur 
(\texttt{config.hjson}) :
...
Lors de l'exécution du script de provisionnement Python, ce binaire de calibration est lu et injecté en mémoire 
Flash aux côtés des paramètres réseau et du domaine réglementaire (\texttt{"wlan.country\_code": "US"}), permettant à
la puce radio de démarrer et de s'initialiser avec succès.