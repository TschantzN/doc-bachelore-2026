...
\begin{figure}[htbp]
    ...
    \caption{Architecture globale du système de transmission vidéo avec les différentes options testées}
...
\begin{itemize}
    ...
    qui introduisaient plus de latence logicielle que l'implémentation directe de \texttt{rpicam-vid} 
    sur la Raspberry Pi.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{La station de décodage haute performance (PC fixe, GPU NVIDIA RTX 4070 Tis) :} Bien que l'architecture 
...
\begin{itemize}
    ...
    \item \textbf{Côté Récepteur (RX) :} La carte de réception EKH19 reliée au routeur OpenWrt GL.iNet MT3000, configurée en simple pont Ethernet transparent vers notre PC fixe de diagnostic (décodage GStreamer logiciel sans synchronisation et affichage direct sur l'écran haute vitesse 165 Hz).
\end{itemize}
\begin{figure}[htbp]
    \centering
...
\subsection{Précision sur la structure du document}
Comme vu précédemment voici l'ordre dans lequel les différents éléments du système seront développés.%  \ref{fig:complet_schematic_nb}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{imgs/complet_schematic_nb.png}
    \caption{Flux logique des données : (1) Lien réseau Sub-1 GHz, (2) Capture vidéo, (3) Plateformes de traitement, (4) Interfaçage SPI.}
